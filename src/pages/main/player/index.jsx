...
const Player = () => {
    const { songToPlay, songToPlayId, settings, songs, songToPlayFull } = useSelector((state) => state.songs)
    console.log({ songToPlayFull });
    const [color, setColor] = useState("#000000");
    const [loading, setLoading] = useState(false);
    const [fontColor, setFontColor] = useState(songToPlay.font_color);
    const [lineColor, setLineColor] = useState(fontColor);
    const [currentLine, setCurrentLine] = useState(0);
    const [value, setValue] = useState(30);
    const [speedAtStop, setSpeedAtStop] = useState();
    const [fontSize, setfontSize] = useState(songToPlayFull.font_size);
    const [playSpeed, setPlaySpeed] = useState(songToPlayFull.scroll_speed);
    ...
    const [isPlaying, setIsPlaying] = useState(true); // Track if the song is playing
    const [translateY, setTranslateY] = useState(0); // Initial translateY value
    ...
    useEffect(() => {
        ...
        const updatePosition = () => {
            if (isPlaying) {
                setTranslateY(prevY => prevY - 1); // Example of updating position, adjust based on actual speed
    ...
    }, [isPlaying]); // Restart animation when isPlaying changes
        ...
        if (songs.length > 0) {
            ...
            if (index < songs.length - 1) {
                ...
                try {
                    ...
                    setfontSize(song2.data.data.font_size);
                    setPlaySpeed(song2.data.data.scroll_speed);
                    setNewSongToPlay(songLyrics);
                    setLinesToSkip(0);
                } catch (error) {
                    console.log(error);
                } finally {
                ...
                <main id="main" class="layrics-main">
                    ...
                    <div className={fullScreen ? "fullScreenContainerFluid " : "container-fluid controlsScreenFlex"}>
                                ...
                                <div class="control-box">
                                    <div class="frame">
                                        ...
                                        {isFontColorPickerOpen && (
                                            <ColorPicker color={fontColor} setColor={(e) => { setisTrue(false); setFontColor(e); }} onClose={() => setIsFontColorPickerOpen(false)} />
                                        )}
                        ...
                        <div className={fullScreen ? "list-container lyrics-frame fullScreenListContainer" : "list-container lyrics-frame"} style={settings ? { background: color } : { width: "100%", background: color }}>
                            <div className={fullScreen ? "fullScreenIcon d-flex justify-content-end" : 'd-flex justify-content-end nonFullScreenIcon'}>
                            ...
                            <div class="holder" >
                                ...
                                <div class="lyrics-list preLine playerDiv" style={
                                    isPlaying
                                        ? { transform: `translateY(${translateY}px)`, transition: `transform ${emSpeed/100}s, opacity 0.5s`, fontSize: fontSize }
                                        : { fontSize: fontSize, overflow: 'hidden', transform: `translateY(${translateY}px)` }
                                }>
                                    {/* <div class="lyrics-list preLine playerDiv" style={emSpeed > 0 ? { animation: `scrollText ${emSpeed}s linear`, fontSize: fontSize } : { fontSize: fontSize, overflow: 'hidden', transform: `translateY(-${linesToSkip}px)`, transition: 'transform 0.5s, opacity 0.5s' }}> */}
                                        ...
                                        {songToPlay.map((line, index) => (
                                            ...
                                            <li key={index} className="lyrics-line"
                                                style={{
                                                    color: selectedLines.find((lineObj) => lineObj.index === index) ? selectedLines.find((lineObj) => lineObj.index === index).color : line.match(/style="color: (.*?)"/)[1] || color,
